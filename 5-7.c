...
int readlines(char *line_ptr[], int maxlines, char *stored_lines);
...
int main(void) {
    ...
    if ((nlines = readlines(line_ptr, MAXLINES, stored_lines)) >= 0) {
...
int readlines(char *line_ptr[], int maxlines, char *stored_lines) {
    ...
    while ((len = get_line(line, MAXLEN)) > 0) {
        // Checking if the current # of lines exceeds the max # of lines that
        // can be stored Also checking if the max # of chars from the
        // stored_lines buffer is not exceeded
        if (nlines >= maxlines ||stored_lines + MAXSTORE - p < len) {
            return -1;
        } else {
